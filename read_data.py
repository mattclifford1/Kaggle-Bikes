...
def split_training_data_phase3(csv_file, test_size=0.33):
    pd_dataframe = pd.read_csv(csv_file)
    # convert weekdays to ints
    days = {'Monday': 0, 'Tuesday': 1, 'Wednesday': 2, 'Thursday': 3, 'Friday': 4, 'Saturday': 5, 'Sunday': 6}
    pd_dataframe['weekday'] = [days[x] for x in pd_dataframe['weekday']]
    ...
    _, X_test_phase2, _, _ = train_test_split(pd_dataframe.to_numpy(),
                                              pd_dataframe['bikes'].to_numpy(),
                                              test_size=test_size,
                                              random_state=rand_state)
    X_test_phase2_df = pd.DataFrame(data= X_test_phase2,
                                   columns=pd_dataframe.columns)

    return X_train, X_test, y_train, y_test, X_test_phase2_df
